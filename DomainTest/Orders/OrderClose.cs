...
{
    public class OrderClose : UnitTestBase<Order, Guid>
    {
        private OrderService _orderService;

        public OrderClose() : base()
        {
        ...
        public async Task OrderClose_Should_Success(Guid orderId)
        {
            ...
            Order order = await _orderService.OrderClose(orderId);
            Assert.Equal(orderId, order.Id);
            Assert.True(order.IsClose);
        }

        [Theory]
        [InlineData("bbbb4ed8-3e36-40fd-b57e-dfbbcf7e1bb8")]
        public async Task OrderClose_Should_ThrowOrderNotFoundException(Guid orderId)
        {
            _repository.Setup(s => s.GetAsync(orderId)).ReturnsAsync(DataSeed.Orders.FirstOrDefault(o => o.Id == orderId));
            await Assert.ThrowsAsync<OrderNotFoundException>(() => _orderService.OrderClose(orderId));
        }

        [Theory]
        [InlineData("6a7a6f4b-3df2-4018-b37a-4ae7b6272222")]
        public async Task OrderClose_Should_ThrowAlreadlyCloseException(Guid orderId)
        {
            _repository.Setup(s => s.GetAsync(orderId)).ReturnsAsync(DataSeed.Orders.FirstOrDefault(o => o.Id == orderId));
            await Assert.ThrowsAsync<OrderAlreadlyCloseException>(() => _orderService.OrderClose(orderId));
        }
    }
}
